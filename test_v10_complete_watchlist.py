"""
Test COMPLET de TextBlob V10 sur les 40 actions de la watchlist
"""
...
from textblob_v10_final import score_news_v10_final
...
async def test_complete_watchlist():
    print("="*100)
    print("TEST COMPLET V10 - TOUTE LA WATCHLIST (40 ACTIONS)")
    print("="*100)
    ...
    print(f"{'#':<4} {'Symbol':<10} {'V10':>7} {'Claude':>7} {'Diff':>7} {'Status'}")
    print("-"*100)
    ...
    for i, symbol in enumerate(WATCHLIST, 1):
        try:
            ...
            v10_score = score_news_v10_final(news_items)
            claude_score = await scorer.score_with_claude(symbol, news_items)
            diff = abs(claude_score - v10_score)
            ...
            print(f"{i:<4} {symbol:<10} {v10_score:>7.1f} {claude_score:>7.1f} {diff:>7.1f} {status}")
    ...
    print("-"*100)
    print(f"{'Symbol':<10} {'V10':>7} {'Claude':>7} {'Diff':>7}")
    print("-"*100)

    sorted_results = sorted(results, key=lambda x: x['diff'])
    for r in sorted_results[:10]:
        print(f"{r['symbol']:<10} {r['v10']:>7.1f} {r['claude']:>7.1f} {r['diff']:>7.1f}")
    ...
    print()
    print("="*100)
    print("COMPARAISON vs BASELINES:")
    print("="*100)
    print(f"Baseline V1:    8.7 pts moyenne")
    print(f"V9 Refined:     10.3 pts moyenne")
    print(f"V10 Final:      {avg_diff:.1f} pts moyenne")
    print()

    # V9 comparison
    improvement_v9 = 10.3 - avg_diff
    if improvement_v9 > 0:
        print(f"AMELIORATION vs V9:   {improvement_v9:.1f} pts ({improvement_v9/10.3*100:.1f}%)")
    else:
        print(f"Pas d'amelioration vs V9 ({improvement_v9:.1f} pts)")

    # V1 comparison
    improvement_v1 = 8.7 - avg_diff
    if improvement_v1 > 0:
        print(f"AMELIORATION vs V1:   {improvement_v1:.1f} pts ({improvement_v1/8.7*100:.1f}%)")
    else:
        print(f"Regression vs V1 ({improvement_v1:.1f} pts)")
    ...
    if avg_diff <= 5:
        ...
        print("V10 est quasi identique a Claude!")
    elif avg_diff <= 6:
        print(f"\n*** OBJECTIF ATTEINT! Ecart moyen de {avg_diff:.1f} pts ***")
        print("V10 rivalise avec Claude!")
    elif avg_diff <= 8:
        print(f"\nTres bon! Ecart moyen de {avg_diff:.1f} pts")
        print("V10 est bien calibre")
    else:
