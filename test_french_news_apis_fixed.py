"""
Test de diffÃ©rentes APIs pour rÃ©cupÃ©rer les news franÃ§aises (CAC 40)
...
async def test_alpha_vantage(symbol: str):
    """
    Alpha Vantage - Gratuit (25 requÃªtes/jour)
            ...
            async with session.get(url, params=params, timeout=10) as response:
                if response.status == 200:
                    ...
                    if 'feed' in data:
                        ...
                        print(f"   âœ… {news_count} news trouvÃ©es")
                        ...
                        print(f"   âŒ Pas de news dans la rÃ©ponse")
                        print(f"   RÃ©ponse: {data}")
                        return False, 0
                else:
                    print(f"   âŒ Erreur {response.status}")
...
async def test_newsapi_french(symbol: str):
    ...
    On l'utilise dÃ©jÃ  mais testons avec meilleurs termes de recherche
    """
    ...
    if not api_key:
        print("   âŒ Pas de clÃ© NewsAPI dans .env")
        return False, 0
            ...
            async with session.get(url, params=params, timeout=10) as response:
                if response.status == 200:
                    ...
                    if data.get('status') == 'ok':
                        ...
                        print(f"   âŒ Status: {data.get('status')}")
                        return False, 0
                else:
                    print(f"   âŒ Erreur {response.status}")
                    return False, 0
            ...
            async with session.get(url, params=params, timeout=10) as response:
                if response.status == 200:
                    ...
                    if isinstance(data, list):
                        ...
                        print(f"   âŒ Format de rÃ©ponse inattendu")
                        print(f"   {data}")
                        return False, 0
                else:
                    print(f"   âŒ Erreur {response.status}")
...
async def test_marketaux(symbol: str):
    """
    Marketaux - API gratuite spÃ©cialisÃ©e news financiÃ¨res
    """
...
async def main():
    print("=" * 100)
    print("TEST DES APIs POUR NEWS FRANÃ‡AISES (CAC 40)")
    print("=" * 100)
    print("\nObjectif: Trouver une alternative Ã  Finnhub qui donne des 403 sur les actions .PA")
    print("\nActions testÃ©es:")
    ...
    if best_total > 0:
        print(f"\nâœ… Meilleure API: {best_name} ({best_total} news au total)")
        print(f"\nUtilise {best_name} comme source secondaire pour les actions franÃ§aises (.PA)")
        print(f"Finnhub reste pour les actions US (AAPL, MSFT, etc.)")
    else:
        print("\nâŒ Aucune API gratuite ne fonctionne bien pour les actions franÃ§aises")
        print("\nSolution: Continuer avec NewsAPI uniquement (dÃ©jÃ  utilisÃ©)")
